...
export default function InsertDeviceTypePage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to insert device type");
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center font-sans">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-md p-8 w-full max-w-lg flex flex-col mt-20 mb-10">
        <h1 className="text-2xl font-bold mb-6 text-center">Insert Device Type</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block mb-1 font-medium text-gray-700">Type Name</label>
            <input name="type_name" value={form.type_name} onChange={handleChange} className="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200" required autoFocus />
          </div>
          <div>
          ...
          <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition" disabled={loading}>
            {loading ? "Saving..." : "Insert Device Type"}
          </button>
        </form>
      </div>
    </div>
